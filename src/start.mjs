import { badRequest, requestWithError, goodRequest } from './api'
import * as util from 'util'
//NOTE about above:
//In order to use ES6 modules with node, you either have to
//transpile with Babel, or you can enable experimental 
//modules support in node. If you do the latter, the file
//name must end in an .mjs extension (see https://nodejs.org/api/esm.html)
...
async function main () {
  ...
  //etc.
  const iffyPromise = new Promise( (resolve, reject) => {
  ...
  console.log("\n\n")
  console.log('*************************************')
  console.log("Examples where we externalized to an API:\n")
  //API examples
  //Request that can't connect
  try {
    const response1 = await badRequest();
    //This shouldn't fire
    console.log(`Response 1 is ${response1}`)
  } catch (error) {
    //console.log(`In main file, catching error: ${util.inspect(error)}`)
    console.log(`In main file, catching error: ${error}`)
  }

  //Request that returns error code
  try {
    const response2 = await requestWithError();
    //This shouldn't fire
  ...
  try {
    const response3 = await goodRequest();
    console.log(`Response 3 is ${response3}`)
  } catch (error) {
    //This won't ordinarily fire -- unless there is a genuine error!
    console.log(`In main file, catching error: ${error}`)
  }
}
